...
- [x] Cross-cell replication (opt-in, policy-driven). `migration/cross_cell/replicator.go` mirrors manifests whose `PlacementPolicy.ReplicationPolicy.Mode == "async"` from a source provider to a destination provider; `internal/config.CrossCellConfig` gates the worker. (Devin Review flagged that the replica manifest's `Piece.Backend` still references the source cell ID; follow-up tracked.)
